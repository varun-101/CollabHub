...
app.post("/git", async (req, res) => {
    try {
        ...
        if(sort == "All")
        {
            result = await axios.get(`https://api.github.com/search/repositories?q=${query}`, config);
        }
        else{
            result = await axios.get(`https://api.github.com/search/repositories?q=${query}&sort=${sort}`, config);
